                        ...
                        <div class="card-body" style="padding: 40px;">
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 40px;">
                                ...
                                <div style="text-align: center; min-width: 140px;">
                                    ...
                                    <div style="font-size: 16px;">
                                        No. <span style="border-bottom: 1px solid black; display: inline-block; width: 180px; text-align: center; padding-bottom: 2px;">{{ $no_kwitansi ?? ($pembayaran_list[0]['nomor_kwitansi'] ?? '') }}</span>
                                    </div>
                                    @if(isset($pembayaran_list) && is_array($pembayaran_list) && count($pembayaran_list) > 0)
                                        @php
                                            $metodeBayar = $pembayaran_list[0]['metode_bayar'] ?? '';
                                            $metodeBayarText = '';
                                            switch($metodeBayar) {
                                                case 'tunai':
                                                ...
                                                case 'tabungan':
                                                    ...
                                                    break;
                                                default:
                                                    $metodeBayarText = ucfirst($metodeBayar);
                                            }
                                        @endphp
                            ...
                            <div style="border: 2px solid black; padding: 30px; margin-bottom: 40px;">
                                ...
                                <div style="display: flex; margin-bottom: 20px; align-items: center;">
                                    ...
                                    <span style="border-bottom: 2px solid black; flex: 1; min-height: 25px; padding: 2px 8px; font-size: 16px;">{{ $siswa->nama ?? 'Nama Siswa Tidak Tersedia' }}</span>
                                </div>
                                ...
                                <div style="margin-bottom: 25px;">
                                    <div style="display: flex; margin-bottom: 15px; align-items: flex-start;">
                                        ...
                                        <span style="margin: 0 10px; font-size: 16px;">:</span>
                                        <div style="flex: 1;">
                                            <!-- Item pembayaran pertama langsung di samping -->
                                            @if(isset($pembayaran_list) && count($pembayaran_list) > 0)
                                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                                                    <span style="border-bottom: 2px solid black; flex: 1; min-height: 25px; padding: 2px 8px; font-size: 16px; margin-right: 20px;">1. {{ convertBulanToIndonesia($pembayaran_list[0]['nama_tagihan'] ?? 'Pembayaran') }}</span>
                                                    <span style="font-size: 16px; font-weight: bold;">Rp {{ number_format($pembayaran_list[0]['jumlah_asli'] ?? ($pembayaran_list[0]['jumlah'] ?? 0), 0, ',', '.') }}</span>
                                                    <input type="hidden" name="nomor_kwitansi" value="{{ $no_kwitansi ?? ($pembayaran_list[0]['nomor_kwitansi'] ?? '') }}">
                                                </div>
                                                <!-- Diskon untuk item pertama -->
                                                @if(isset($pembayaran_list[0]['diskon']) && $pembayaran_list[0]['diskon'] > 0)
                                                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                                                        ...
                                                        <span style="font-size: 16px; font-weight: bold;">Rp {{ number_format($pembayaran_list[0]['diskon'], 0, ',', '.') }}</span>
                                                    </div>
                                                @endif
                                            @else
                                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                                                    <span style="border-bottom: 2px solid black; flex: 1; min-height: 25px; padding: 2px 8px; font-size: 16px; margin-right: 20px;">1. SPP Bulan</span>
                                            ...
                                            @for($i = 1; $i < count($pembayaran_list); $i++)
                                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                                                    @if(isset($pembayaran_list[$i]))
                                                        <span style="border-bottom: 2px solid black; flex: 1; min-height: 25px; padding: 2px 8px; font-size: 16px; margin-right: 20px;">{{ $i + 1 }}. {{ convertBulanToIndonesia($pembayaran_list[$i]['nama_tagihan'] ?? 'Pembayaran') }}</span>
                                                        <span style="font-size: 16px; font-weight: bold;">Rp {{ number_format($pembayaran_list[$i]['jumlah_asli'] ?? ($pembayaran_list[$i]['jumlah'] ?? 0), 0, ',', '.') }}</span>
                                                        <input type="hidden" name="nomor_kwitansi" value="{{ $no_kwitansi ?? ($pembayaran_list[0]['nomor_kwitansi'] ?? '') }}">
                                                    @else
                                                ...
                                                <!-- Diskon untuk item lainnya -->
                                                @if(isset($pembayaran_list[$i]) && isset($pembayaran_list[$i]['diskon']) && $pembayaran_list[$i]['diskon'] > 0)
                                                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                                                        <span style="border-bottom: 2px solid black; flex: 1; min-height: 25px; padding: 2px 8px; font-size: 16px; margin-right: 20px; margin-left: 20px;">Potongan</span>
                                                        <span style="font-size: 16px; font-weight: bold;">Rp {{ number_format($pembayaran_list[$i]['diskon'], 0, ',', '.') }}</span>
                                                    </div>
                                                @endif
                                            @endfor
                                        @endif
                                    ...
                                    @php
                                        ...
                                        if (isset($pembayaran_list) && is_array($pembayaran_list)) {
                                            foreach ($pembayaran_list as $pembayaran) {
                                                if (isset($pembayaran['jumlah'])) {
                                                    $jumlah += $pembayaran['jumlah'];
                                                }
                                                if (isset($pembayaran['diskon'])) {
                                                    $potongan += $pembayaran['diskon'];
                                                }
                                            }
                                        }
                                        
                                        // Perbaiki perhitungan: jumlah adalah total sebelum potongan, jumlahDiterima adalah setelah potongan
                                        $jumlahDiterima = $jumlah; // Jumlah diterima adalah jumlah bayar
                                        $jumlah = $jumlahDiterima + $potongan; // Jumlah adalah jumlah diterima + potongan
                                    @endphp
                                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            ...
                            <div style="text-align: right; margin-top: 60px;">
                                ...
                                @endphp
                                <p style="margin-bottom: 80px; font-size: 16px;">Jambi, {{ \Carbon\Carbon::now()->translatedFormat('d F Y') }}</p>
...
// Tambahkan fungsi convertBulanToIndonesia jika belum ada
function convertBulanToIndonesia(text) {
    var months = {
        'January': 'Januari',
        'February': 'Februari',
        'March': 'Maret',
    ...
    Object.keys(months).forEach(function(englishMonth) {
        text = text.replace(new RegExp(englishMonth, 'g'), months[englishMonth]);
    });
