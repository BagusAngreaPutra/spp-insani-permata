...
<div class="main-content">
    ...
    <div class="content-area">
        <div class="form-container">
            <h2><i class="fas fa-calendar-alt"></i> Edit Tahun Ajaran</h2>

            <form action="{{ route('tahun_ajaran.update', $tahun_ajaran->id) }}" method="POST">
                @csrf
                @method('PUT')
                <label for="nama_tahun" class="form-label">Nama Tahun</label>
                <input type="text" id="nama_tahun" name="nama_tahun" class="form-control"
                       value="{{ $tahun_ajaran->nama_tahun }}" required>

                <label class="form-label inline-flex items-center">
                    <input type="checkbox" name="aktif" value="1" class="mr-2" {{ $tahun_ajaran->aktif ? 'checked' : '' }}>
                    Aktif
                </label>
